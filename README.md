...
- FavoriteWidget 用户收藏组件，用于在应用中将一些内容进行收藏纪录。
...
- sourceType: 积分的来源信息，比如来自集群，模型还是用户信息。目前支持的类型："user", "cluster", "worker", "model"
- sourceId : 来源信息的唯一标识，比如用户ID，集群ID，模型ID等
- onClick: 点击收藏图标后的回调方法，用于处理点击后的一些事件，比如刷新数据等
